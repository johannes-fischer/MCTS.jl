...
POMDPs.solve(solver::GenBeliefRealDPWSolver, mdp::Union{POMDP, MDP}) =
    GenBeliefRealDPWPlanner(solver, mdp)

"""
Delete existing decision tree.
"""
function clear_tree!(p::GenBeliefRealDPWPlanner)
    p.tree = nothing
end

"""
Construct an MCTSGenBeliefRealDPW tree and choose the best action.
"""
POMDPs.action(p::GenBeliefRealDPWPlanner, b) = first(action_info(p, b))
...
function POMDPModelTools.action_info(
    ...
    try
        ...
        #     end
        # else
        tree = GenBeliefRealDPWTree{B, A}(p.solver.n_iterations)
        p.tree = tree
...
"""
Return the reward for one iteration of MCTSGenBeliefRealDPW.
"""
function simulate(dpw::GenBeliefRealDPWPlanner, snode::Int, d::Int, s, b)
    # S = statetype(dpw.mdp)
